...
![image-20240124182106971](./Hexo+Butterfly自定义右键菜单/images/image-20240124182106971.png)
...
在[博客根目录]/themes/butterfly/layout/includes文件夹下新建一个right-menu的文件夹，在此文件夹下新建一个index.pug文件。

![image-20240124182330005](./Hexo+Butterfly自定义右键菜单/images/image-20240124182330005.png)
...
在[博客根目录]/themes/butterfly/layout/includes/layout.pug中引入上一步中创建的index.pug文件。
具体位置如下图：

![image-20240124182726104](./Hexo+Butterfly自定义右键菜单/images/image-20240124182726104.png)
...
在[博客根目录]/themes/butterfly/source/js文件夹下新建一个rightMenu.js，将以下代码复制到文件中。
...
在[博客根目录]/themes/butterfly/source/css文件夹下新建一个rightMenu.css，将以下代码复制到文件中。
...
在主题配置文件[博客根目录]/themes/butterfly/_config.yml中引入Jquery、rightMenu.js和rightMenu.css
...
![image-20240124183415110](./Hexo+Butterfly自定义右键菜单/images/image-20240124183415110.png)
...
1. 增加DOM。（为了描述的更清晰，将沿用上面提到过的代码，+表示在此基础上新增的代码。）
   在[博客根目录]/themes/butterfly/layout/includes/right-menu/index.pug中新加如下代码：
...
有兴趣的同学可以按下`F12`打开控制台，找到Elements，并找到#rightMenu的盒子，你会发现新增的pug语法最终会被编译成：
...
![image-20240124183656924](./Hexo+Butterfly自定义右键菜单/images/image-20240124183656924.png)

记住这个`id`为`menu-print`的属性，下面将会用到。



2. 在`[博客根目录]/themes/butterfly/source/js/rightMenu.js`中写入实现方法。
...
不难发现，新增的代码实际上是在`id`为`menu-print`的盒子上绑了一个点击事件，后面的方法则是触发点击事件后要执行的过程。
...
3. 此时点击鼠标右键，会出现新加的一项功能`打印页面`。

![image-20240124183933695](./Hexo+Butterfly自定义右键菜单/images/image-20240124183933695.png)

到此，你学废了吗？遇到问题的话请在评论区留言。



文章整合引用：[Hexo + Butterfly 自定义右键菜单 | 唐志远 (fe32.top)](https://fe32.top/articles/hexo1608/)